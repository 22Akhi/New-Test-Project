...
jQuery( document ).ready(function($) {
		...
		if(numRepeaterItems > 0) {
			...
			if(numRepeaterItems > 1) {
				...
				for (i = 1; i < numRepeaterItems; ++i) {
					radiantBusinessAppendRow($(this), defaultValuesArray[i]);
				}
			}
		}
	});
	...
	$(this).find('.sortable_repeater.sortable').sortable({
		update: function(event, ui) {
			radiantBusinessGetAllInputs($(this).parent());
		}
	});
	...
	$('.sortable_repeater.sortable').on('click', '.customize-control-sortable-repeater-delete', function(event) {
		...
		if(numItems > 1) {
			$(this).parent().slideUp('fast', function() {
				...
				$(this).remove();
				radiantBusinessGetAllInputs(parentContainer);
			})
		}
		else {
			$(this).parent().find('.repeater-input').val('');
			radiantBusinessGetAllInputs($(this).parent().parent().parent());
		}
	});
	...
	$('.customize-control-sortable-repeater-add').on( 'click', function(event) {
		event.preventDefault();
		radiantBusinessAppendRow($(this).parent(), '');
		radiantBusinessGetAllInputs($(this).parent());
	});
	...
	function radiantBusinessAppendRow($element, defaultValue) {
	...
	function radiantBusinessGetAllInputs($element) {
	...
	$('.multi-image-checkbox').on('change', function () {
	  radiantBusinessGetAllImageCheckboxes($(this).parent().parent());
	});

	// Get the values from the checkboxes and add to our hidden field
	function radiantBusinessGetAllImageCheckboxes($element) {
	...
	$('.customize-control-tinymce-editor').each(function(){
		...
		var tinyMCEToolbar1String = _wpCustomizeSettings.controls[$(this).attr('id')].radiantBusinesstinymcetoolbar1;
		var tinyMCEToolbar2String = _wpCustomizeSettings.controls[$(this).attr('id')].radiantBusinesstinymcetoolbar2;
		var tinyMCEMediaButtons = _wpCustomizeSettings.controls[$(this).attr('id')].radiantBusinessmediabuttons;
 ...
 */
( function( $, api ) {
	api.sectionConstructor['radiant-business-upsell'] = api.Section.extend( {
